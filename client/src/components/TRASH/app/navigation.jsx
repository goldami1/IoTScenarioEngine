...
class StarterNavigation extends React.Component {
	...
	links(type) {
		switch (type) {
			case false:
				return [["/products", "Products", DashboardIcon]];
			case true:
				return [
		...
		return (
			<Navigation
				...
				containerHeaderComponent={() => (
					<AkContainerTitle
						href="#foo"
						icon={<img alt="nucleus" src={nucleusImage} />}
				...
				globalSecondaryActions={[
					<UserDropdown onLogout={this.props.logout} />
				]}
			...
			>
				{this.links(this.props.auth.user.isCustomer).map(link => {
					...
					return (
						<Tooltip
							content={
								!this.context.navOpenState.isOpen ? title : ""
							}
							position="right"
						>
							<Link key={url} to={url}>
